...
import { handleDeleteRecipe, handleExtractRecipe, handleGetSharedRecipe, handleListRecipes, handleSaveRecipe, handleShareRecipe } from './handlers/recipes.ts';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    else if (url.pathname.startsWith('/families/recipes/') && url.pathname.endsWith('/share') && request.method === 'POST') {
      const recipeId = url.pathname.slice('/families/recipes/'.length, -'/share'.length);
      response = await handleShareRecipe(request, env, recipeId);
    }
    ...
    // ── Public routes ──────────────────────────────────────────────────────────
    else if (url.pathname === '/shared/recipe' && request.method === 'GET')
      response = await handleGetSharedRecipe(request, env);
